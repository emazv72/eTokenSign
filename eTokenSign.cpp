...
int wmain(int argc, wchar_t** argv) {
	...
	// --- Resolve certFile relative to the executable path ---
	wchar_t exePathBuffer[MAX_PATH];
	::GetModuleFileNameW(NULL, exePathBuffer, MAX_PATH);
	std::wstring exeDir = exePathBuffer;

	// Find the last backslash to isolate the directory (Windows only)
	size_t lastSlashPos = exeDir.rfind(L'\\');
	if (lastSlashPos != std::wstring::npos) {
		exeDir = exeDir.substr(0, lastSlashPos + 1); // Keep the trailing slash
	}
	else {
		exeDir = L"";
	}

	std::wstring absoluteCertFile = exeDir + certFile;


	// 1. Load the standalone .cer file into memory
	PCCERT_CONTEXT pCertContext = NULL;
	if (!::CryptQueryObject(CERT_QUERY_OBJECT_FILE, absoluteCertFile.c_str(),
